...
| POST | `/v1/card-authorization-procedure/initiate` | Initiate |
| GET | `/v1/card-authorization-procedure` | Retrieve (list) |
| GET | `/v1/card-authorization-procedure/{crId}/retrieve` | Retrieve |
| PUT | `/v1/card-authorization-procedure/{crId}/update` | Update |
| PUT | `/v1/card-authorization-procedure/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/card-authorization-procedure/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-card-authorization:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-card-authorization ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-card-authorization` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
